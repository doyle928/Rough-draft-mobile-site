...
(function ($, window, document, undefined) {
	var pluginName = "parallaxify",
		...
		positionProperty = {
			...
			transform: {
				setPosition: function ($element, left, originalLeft, top, originalTop) {
					$element[0].style[prefixedTransform] =
						"translate3d(" +
						(left - originalLeft) +
						"px, " +
						(top - originalTop) +
		...
		function ($element, x, y) {
			...
			let yFinal = y2 + "px";
			$element.css({
				"background-position-x": x + ", " + xFinal,
				"background-position-y": y + ", " + yFinal
			});
		} :
		function ($element, x, y) {
	...
	Plugin.prototype = {
		...
		_defineElements: function () {
			this.$element =
				this.element === document.body || this.element === window ?
				$("body") :
				$(this.element);
		...
		_findElements: function () {
			...
			if (this.elements !== undefined) {
				for (var i = this.elements.length - 1; i >= 0; i--) {
					this.elements[i].$element.data(
						"parallaxify-ElementIsActive",
						undefined
					);
				}
			}
			...
			this.$element
				...
				.each(function (i) {
					...
					self.elements.push({
						...
						parallaxDistanceX: $this.data("parallaxify-range-x") !== undefined ?
							$this.data("parallaxify-range-x") : $this.data("parallaxify-range") !== undefined ?
							$this.data("parallaxify-range") : 0,
						parallaxDistanceY: $this.data("parallaxify-range-y") !== undefined ?
							$this.data("parallaxify-range-y") : $this.data("parallaxify-range") !== undefined ?
							$this.data("parallaxify-range") : 0,
		...
		_findBackgrounds: function () {
			...
			$backgroundElements.each(function () {
				...
				if (!$this.data("parralaxify-backgroundOriginalLeft")) {
					...
					$this.data(
						"parallaxify-backgroundOriginalTop",
						backgroundPosition[1]
					);
				} else {
					setBackgroundPosition(
						$this,
						$this.data("parallaxify-backgroundOriginalLeft"),
						$this.data("parallaxify-backgroundOriginalTop")
					);
				}
				...
				self.backgrounds.push({
					...
					originalBackgroundPositionLeft: isNaN(
						...
						0 : parseInt(backgroundPosition[0], 10),
					originalBackgroundPositionTop: isNaN(
							parseInt(backgroundPosition[1], 10)
						) ?
						0 : parseInt(backgroundPosition[1], 10),
					originalPositionLeft: $this.position().left,
					originalPositionTop: $this.position().top,
					parallaxDistanceX: $this.data("parallaxify-background-range-x") !== undefined ?
						$this.data("parallaxify-background-range-x") : $this.data("parallaxify-background-range") !== undefined ?
						$this.data("parallaxify-background-range") : 0,
					parallaxDistanceY: $this.data("parallaxify-background-range-y") !== undefined ?
						$this.data("parallaxify-background-range-y") : $this.data("parallaxify-background-range") !== undefined ?
						$this.data("parallaxify-background-range") : 0
				});
			});
		},
		_reset: function () {
			...
			for (i = this.elements.length - 1; i >= 0; i--) {
				...
				originalPositionLeft = element.$element.data(
					"parallaxify-originalLeft"
				);
		...
		_processSensorData: function () {
			...
			deltaBeta =
				this.initialBeta !== undefined ? beta - this.initialBeta : beta;
			deltaGamma =
	...
	$[pluginName] = function (options) {
		var $window = $(window);
		return $window[pluginName].apply(
			$window,
